...
| 2 — State reconstruction | Block-atomic and checkpoint-contract foundation implemented; stage not passed | Pure synchronous reducer contract, default-deny kind/schema ownership, contiguous committed watermark, deterministic canonical state bytes and hash, duplicate idempotence, whole-block rollback, bounded mutations, immutable state deltas, exact state-image restore, content-derived canonical checkpoint manifests bound to archive/schema/reducer identity, and focused adversarial tests | Qualified action-bearing semantic reducers, crash-safe filesystem/RocksDB checkpoint adapters, archive replay runtime, correction handling, account/book reconciliation, rebuild evidence, and signed gate |
