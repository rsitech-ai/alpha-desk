...
if grep -Ev \
    '^(paste v1\.0\.15 \(proc-macro\)|parquet v58\.4\.0|datafusion(-[a-z-]+)? v54\.1\.0( \(\*\))?|canonical-archive v0\.1\.0 \(.+\)|replay-engine v0\.1\.0 \(.+\)|hl-analytics v0\.1\.0 \(.+\)|hl-capture v0\.1\.0 \(.+\)|archive-inspect v0\.1\.0 \(.+\)|spool-inspect v0\.1\.0 \(.+\))$' \
