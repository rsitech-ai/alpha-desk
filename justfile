...
state-replay-position-e2e blocks="30" checkpoint_after="12" iterations="4":
    run_id="$(date -u +%Y%m%dT%H%M%SZ)-$$"; output="target/evidence/state-replay-position/$run_id"; mkdir -p "target/evidence/state-replay-position"; cargo +1.97.1 run -p state-replay --locked --offline -- position-e2e --output "$output" --blocks {{quote(blocks)}} --checkpoint-after {{quote(checkpoint_after)}} --iterations {{quote(iterations)}}; printf 'state-replay-position-report:%s/report.json\n' "$output"

state-replay-position-soak blocks="1000" checkpoint_after="500" iterations="100":
    run_id="$(date -u +%Y%m%dT%H%M%SZ)-$$"; output="target/evidence/state-replay-position/$run_id"; mkdir -p "target/evidence/state-replay-position"; cargo +1.97.1 run --release -p state-replay --locked --offline -- position-e2e --output "$output" --blocks {{quote(blocks)}} --checkpoint-after {{quote(checkpoint_after)}} --iterations {{quote(iterations)}}; printf 'state-replay-position-soak-report:%s/report.json\n' "$output"
