...
/// True when top-level `CaptureStatusBase.properties.oldest_pending_capture_height`
/// is an optional u64 integer: `type: integer`, `minimum: 0`, not listed on
/// `CaptureStatusBase.required`, and no `$ref`, `enum`, `format`, `pattern`,
/// or `maximum`. Capture writer emits top-level `oldest_pending_capture_height`
/// as `Option<u64>` with `skip_serializing_if`; this crate does not invent extra
/// numeric bounds. HealthAssessment.reason_code stays a free string so
/// unknown RED is not closed out.
#[must_use]
pub fn capture_status_oldest_pending_capture_height_is_optional_u64(document: &str) -> bool {
    let Some(mapping) = yaml_mapping(
        document,
        &[
            "components",
...
"#;
    ...
    #[test]
    fn prose_mention_does_not_satisfy_top_level_oldest_pending_capture_height_optional_u64_freeze()
    {
        let prose_only = r#"
components:
  schemas:
    ...
    CaptureStatusBase:
      properties:
        pending_blocks:
          description: >
            oldest_pending_capture_height remains in prose after the YAML property drops it.
          type: integer
          minimum: 0
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(prose_only),
            "prose mention of oldest_pending_capture_height must not satisfy the optional-u64 freeze"
        );
...
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(nested_only),
            "nested oldest_pending_capture_height must not satisfy the top-level optional-u64 freeze"
        );
...
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(required_integer),
            "required oldest_pending_capture_height must not satisfy the optional-u64 freeze"
        );
...
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(string_height),
            "optional non-integer oldest_pending_capture_height must not satisfy the freeze"
        );
...
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(formatted),
            "invented oldest_pending_capture_height format must not satisfy the freeze"
        );
...
"#;
        assert!(
            !capture_status_oldest_pending_capture_height_is_optional_u64(bounded),
            "invented oldest_pending_capture_height maximum must not satisfy the freeze"
        );
