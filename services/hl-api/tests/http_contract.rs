...
#[tokio::test]
async fn nested_auxiliary_tail_cursor_epoch_is_optional_string() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status-v5.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted nested tail_cursor_epoch must stay 200"
    );
    assert!(
        body["auxiliary_sources"][0]
    ...
    value["auxiliary_sources"][0]["tail_cursor_epoch"] = serde_json::json!("node-file-v1:epoch");
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode known string tail_cursor_epoch"),
    )
    .expect("write known string tail_cursor_epoch");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    let (status, body) = call(&state, "/v1/capture/status", &[]).await;
    assert_eq!(status, 200, "known string tail_cursor_epoch must stay 200");
    assert_eq!(
        body["auxiliary_sources"][0]["tail_cursor_epoch"],
        "node-file-v1:epoch"
    );
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted tail_cursor_epoch"),
    )
    .expect("write omitted tail_cursor_epoch");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted nested tail_cursor_epoch after removal must stay 200"
    );
    assert!(
        body["auxiliary_sources"][0]
            .get("tail_cursor_epoch")
    ...
    for tail_cursor_epoch in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-string tail_cursor_epoch"),
        )
        .expect("write non-string tail_cursor_epoch");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{tail_cursor_epoch} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
#[tokio::test]
async fn nested_auxiliary_unread_bytes_is_optional_u64() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status-v5.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    let (status, body) = call(&state, "/v1/capture/status", &[]).await;
    assert_eq!(status, 200, "omitted nested unread_bytes must stay 200");
    assert!(body["auxiliary_sources"][0].get("unread_bytes").is_none());
    assert_eq!(body["schema_version"], "hl.capture.status.v5");
    ...
    for unread_bytes in [0_u64, 47, u64::MAX] {
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode known u64 unread_bytes"),
        )
        .expect("write known u64 unread_bytes");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 200, "{unread_bytes} must stay 200");
        assert_eq!(body["auxiliary_sources"][0]["unread_bytes"], unread_bytes);
    }
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted unread_bytes"),
    )
    .expect("write omitted unread_bytes");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted nested unread_bytes after removal must stay 200"
    );
    assert!(body["auxiliary_sources"][0].get("unread_bytes").is_none());
    ...
    for unread_bytes in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-u64 unread_bytes"),
        )
        .expect("write non-u64 unread_bytes");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{unread_bytes} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
#[tokio::test]
async fn nested_auxiliary_quarantine_reason_is_optional_string() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status-v5.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted nested quarantine_reason must stay 200"
    );
    assert!(
        body["auxiliary_sources"][0]
    ...
    value["auxiliary_sources"][0]["quarantine_reason"] = serde_json::json!("source.schema_drift");
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode known string quarantine_reason"),
    )
    .expect("write known string quarantine_reason");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    let (status, body) = call(&state, "/v1/capture/status", &[]).await;
    assert_eq!(status, 200, "known string quarantine_reason must stay 200");
    assert_eq!(
        body["auxiliary_sources"][0]["quarantine_reason"],
        "source.schema_drift"
    );
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted quarantine_reason"),
    )
    .expect("write omitted quarantine_reason");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted nested quarantine_reason after removal must stay 200"
    );
    assert!(
        body["auxiliary_sources"][0]
            .get("quarantine_reason")
    ...
    for quarantine_reason in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-string quarantine_reason"),
        )
        .expect("write non-string quarantine_reason");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{quarantine_reason} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
#[tokio::test]
async fn top_level_capture_last_error_reason_is_optional_string() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted top-level last_error_reason must stay 200"
    );
    assert!(body.get("last_error_reason").is_none());
    assert_eq!(body["schema_version"], "hl.capture.status.v4");
    ...
    let (status, body) = call(&state, "/v1/capture/status", &[]).await;
    assert_eq!(status, 200, "known string last_error_reason must stay 200");
    assert_eq!(body["last_error_reason"], "capture_bus.unavailable");
    ...
    assert_eq!(
        status, 200,
        "omitted top-level last_error_reason after removal must stay 200"
    );
    assert!(body.get("last_error_reason").is_none());
...
#[tokio::test]
async fn top_level_capture_failover_height_is_optional_u64() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted top-level failover_height must stay 200"
    );
    assert!(body.get("failover_height").is_none());
    assert_eq!(body["schema_version"], "hl.capture.status.v4");
    ...
    for failover_height in [0_u64, 47, u64::MAX] {
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode known u64 failover_height"),
        )
        .expect("write known u64 failover_height");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 200, "{failover_height} must stay 200");
        assert_eq!(body["failover_height"], failover_height);
    }
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted failover_height"),
    )
    .expect("write omitted failover_height");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted top-level failover_height after removal must stay 200"
    );
    assert!(body.get("failover_height").is_none());
    ...
    for failover_height in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-u64 failover_height"),
        )
        .expect("write non-u64 failover_height");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{failover_height} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
#[tokio::test]
async fn top_level_capture_durable_height_is_optional_u64() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted top-level durable_height must stay 200"
    );
    assert!(body.get("durable_height").is_none());
    assert_eq!(body["schema_version"], "hl.capture.status.v4");
    ...
    for durable_height in [0_u64, 47, u64::MAX] {
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode known u64 durable_height"),
        )
        .expect("write known u64 durable_height");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 200, "{durable_height} must stay 200");
        assert_eq!(body["durable_height"], durable_height);
        assert!(
            body.get("failover_height").is_none(),
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted durable_height"),
    )
    .expect("write omitted durable_height");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted top-level durable_height after removal must stay 200"
    );
    assert!(body.get("durable_height").is_none());
    ...
    for durable_height in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-u64 durable_height"),
        )
        .expect("write non-u64 durable_height");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{durable_height} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
#[tokio::test]
async fn top_level_capture_backlog_records_is_required_u64() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "known u64 capture_backlog_records must stay 200"
    );
    assert_eq!(body["capture_backlog_records"], 0);
    assert_eq!(body["schema_version"], "hl.capture.status.v4");
    assert!(
    ...
    for capture_backlog_records in [12_u64, u64::MAX] {
        value["capture_backlog_records"] = serde_json::json!(capture_backlog_records);
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode known u64 capture_backlog_records"),
        )
        .expect("write known u64 capture_backlog_records");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 200, "{capture_backlog_records} must stay 200");
        assert_eq!(body["capture_backlog_records"], capture_backlog_records);
        assert!(
            body.get("durable_height").is_none(),
            "typing capture_backlog_records must not couple it to durable_height"
        );
    ...
    for capture_backlog_records in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-u64 capture_backlog_records"),
        )
        .expect("write non-u64 capture_backlog_records");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{capture_backlog_records} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted capture_backlog_records"),
    )
    .expect("write omitted capture_backlog_records");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 503,
        "omitted top-level capture_backlog_records must not fail open"
    );
    assert_eq!(body["schema_version"], "hl.api.error.v1");
    assert_eq!(body["code"], "data_unavailable");
    assert_eq!(body["reason_code"], "snapshot_invalid");
...
#[tokio::test]
async fn top_level_capture_archive_manifest_id_is_optional_string() {
    let directory = tempdir().expect("temporary directory");
    let capture_path = copy_api_fixture(directory.path(), "capture-status.json");
    let mut value: Value =
        serde_json::from_slice(&std::fs::read(&capture_path).expect("read fixture"))
    ...
    assert_eq!(
        status, 200,
        "omitted top-level archive_manifest_id must stay 200"
    );
    assert!(body.get("archive_manifest_id").is_none());
    assert_eq!(body["schema_version"], "hl.capture.status.v4");

    for archive_manifest_id in [
        serde_json::json!("manifest-42"),
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode known string archive_manifest_id"),
        )
        .expect("write known string archive_manifest_id");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        assert_eq!(
            status, 200,
            "{archive_manifest_id} must stay 200; writer 512/trim/control is not copied onto API parse"
        );
        assert_eq!(body["archive_manifest_id"], archive_manifest_id);
        assert_eq!(
            body["capture_backlog_records"], 0,
    ...
    std::fs::write(
        &capture_path,
        serde_json::to_vec(&value).expect("encode omitted archive_manifest_id"),
    )
    .expect("write omitted archive_manifest_id");
    let state = state_from(
        directory.path(),
        "loopback-dev",
        None,
    ...
    assert_eq!(
        status, 200,
        "omitted top-level archive_manifest_id after removal must stay 200"
    );
    assert!(body.get("archive_manifest_id").is_none());
    ...
    for archive_manifest_id in [
        ...
        std::fs::write(
            &capture_path,
            serde_json::to_vec(&value).expect("encode non-string archive_manifest_id"),
        )
        .expect("write non-string archive_manifest_id");
        let state = state_from(
            directory.path(),
            "loopback-dev",
            None,
        ...
        let (status, body) = call(&state, "/v1/capture/status", &[]).await;
        assert_eq!(status, 503, "{archive_manifest_id} must not fail open");
        assert_eq!(body["schema_version"], "hl.api.error.v1");
        assert_eq!(body["code"], "data_unavailable");
        assert_eq!(body["reason_code"], "snapshot_invalid");
    }
...
async fn served_openapi_matches_capture_status_v4_v5_and_503_contract() {
    ...
    assert!(
        auxiliary_source_tail_cursor_epoch_is_optional_string(document),
        "served OpenAPI must define CaptureStatusBase.auxiliary_sources.items.tail_cursor_epoch as an optional string"
    );
    assert!(
        auxiliary_source_durable_offset_is_optional_u64(document),
    ...
    assert!(
        auxiliary_source_unread_bytes_is_optional_u64(document),
        "served OpenAPI must define CaptureStatusBase.auxiliary_sources.items.unread_bytes as an optional u64 integer"
    );
    assert!(
        auxiliary_source_last_durable_wall_micros_is_optional_i64(document),
    ...
    assert!(
        capture_status_last_error_reason_is_optional_string(document),
        "served OpenAPI must define CaptureStatusBase.last_error_reason as an optional string"
    );
    assert!(
        capture_status_failover_height_is_optional_u64(document),
    ...
    assert!(
        capture_status_durable_height_is_optional_u64(document),
        "served OpenAPI must define CaptureStatusBase.durable_height as an optional u64 integer"
    );
    assert!(
        capture_status_capture_backlog_records_is_required_u64(document),
